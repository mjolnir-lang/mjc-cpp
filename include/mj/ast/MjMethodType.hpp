...
/// A method type.
class MjMethodType : public MjType {
private:
...
public:
    ...
    constexpr
    MjMethodType(MjFunctionParameterList *function_parameter_list, MjType *return_type) noexcept :
    ...
    bool is_const_callable() const noexcept {
        return (*_function_parameter_list)[0]->type()->is_const() || (*_function_parameter_list)[0]->type()->is_safe();
    }
    ...
    bool is_volatile_callable() const noexcept {
        return (*_function_parameter_list)[0].type().is_volatile();
    }


    constexpr
    const MjFunctionParameterList &function_parameter_list() const noexcept {
        return *_function_parameter_list;
    }


    constexpr
    MjFunctionParameterList &function_parameter_list() noexcept {
        return *_function_parameter_list;
    }
